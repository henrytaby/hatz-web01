...
export default function Home() {
  const recentPosts = getAllItems("blog").slice(0, 3);
  const recentWork = getAllItems("work").slice(0, 2);
  ...
  return (
    <div className="flex flex-col gap-24 pb-20 w-full relative">
      ...
      <section className="flex flex-col items-start gap-8 mt-16 md:mt-24 w-full relative">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary/20 blur-[120px] rounded-full opacity-30 pointer-events-none -z-10" />
        ...
        <p className="text-lg md:text-2xl text-muted-foreground max-w-2xl leading-relaxed mt-2 font-light">
          Soy Henry Taby. Construyo arquitecturas escalables, interfaces veloces y experiencias digitales <strong className="font-semibold text-foreground">premium</strong>.
        </p>
      ...
      <section className="flex flex-col gap-10 w-full relative z-10">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">Una mirada profunda a mis mejores trabajos técnicos.</p>
        </div>
        ...
        <div className="w-full flex justify-center md:justify-end">
          <Link href="/work" className="text-sm font-bold text-muted-foreground hover:text-foreground inline-flex items-center gap-2 transition-colors group">
            Ver el portafolio entero <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
          </Link>
        </div>
      </section>

      {/* Featured Posts with Visual Hierarchy */}
      <section className="flex flex-col gap-10 w-full relative z-10">
        <div className="flex flex-col gap-2 border-t border-border/50 pt-16">
          ...
          <p className="text-muted-foreground">Artículos sobre patrones, Jamstack y escalabilidad.</p>
        </div>
            ...
            >
              <div className="flex flex-col gap-1">
                 ...
                 {post.frontmatter.summary && (
                   <p className="text-muted-foreground text-sm line-clamp-1 max-w-xl">{post.frontmatter.summary}</p>
                 )}
              </div>
              <time
        ...
        <div className="w-full flex justify-center md:justify-end">
          <Link href="/blog" className="text-sm font-bold text-muted-foreground hover:text-foreground inline-flex items-center gap-2 transition-colors group">
            Leer todos los artículos <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
          </Link>
        </div>
      </section>
    </div>
